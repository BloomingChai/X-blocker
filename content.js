(function initContent() {
  ...
  const queuedArticles = new Set();
  const MASKED_LABEL = "垃圾评论已屏蔽";

  function getReasonLabel(matchResult) {
    if (!matchResult) {
      return MASKED_LABEL;
    }

    if (matchResult.keyword) {
      return `命中：${matchResult.keyword}`;
    }
  ...
  function getProfileData(article) {
    ...
    function extractHandleFromHref(node) {
      const profileLink = node?.querySelector('a[href^="/"][role="link"], a[href^="/"]');
    ...
    if (!normalized) {
      return {
        displayName: "",
        handle: extractHandleFromHref(nameNode),
        hasEmojiNode
      };
    }

    const handleMatch = normalized.match(/@([A-Za-z0-9_]+)/);
    const handle = handleMatch ? `@${handleMatch[1]}` : extractHandleFromHref(nameNode);
  ...
  function ensureRevealOverlay(article, matchResult) {
    ...
    if (overlay) {
      ...
      const detail = overlay.querySelector(".xhb-overlay__reason");
      const reasonLabel = getReasonLabel(matchResult);
      if (label) {
        label.textContent = MASKED_LABEL;
      }
      if (detail) {
        detail.textContent = reasonLabel;
      }
    ...
    reason.className = "xhb-overlay__reason";
    reason.textContent = getReasonLabel(matchResult);
    ...
    overlay.append(meta, reason, button);
  ...
  function markSensitiveNodes(article) {
    ...
    if (!article.querySelector(".xhb-sensitive")) {
      Array.from(article.children).forEach((node) => {
        if (!node.classList.contains("xhb-overlay")) {
          node.classList.add("xhb-sensitive");
        }
      });
    }
  ...
  function processArticle(article) {
    ...
    if (window.XHB.isWhitelistedHandle(profile.handle, settings)) {
      clearMask(article);
      return;
    }
  ...
  function queueArticle(article) {
    if (!(article instanceof HTMLElement) || !article.matches(ARTICLE_SELECTOR)) {
      return;
    }

    queuedArticles.add(article);
  }

  function queueArticleFromNode(node) {
    ...
    if (node instanceof HTMLElement) {
      ...
      const closestArticle = node.closest(ARTICLE_SELECTOR);
      if (closestArticle) {
        queueArticle(closestArticle);
      }

      node.querySelectorAll?.(ARTICLE_SELECTOR).forEach((article) => {
        queueArticle(article);
      });
      return;
    }

    const parentArticle = node.parentElement?.closest(ARTICLE_SELECTOR);
    if (parentArticle) {
      queueArticle(parentArticle);
    }
  }
  ...
  function flushQueue() {
    ...
    const articles = Array.from(queuedArticles);
    queuedArticles.clear();
    ...
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        if (mutation.type === "childList") {
          queueArticleFromNode(mutation.target);
          mutation.addedNodes.forEach((node) => {
            queueArticleFromNode(node);
          });
